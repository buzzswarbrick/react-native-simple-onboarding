...
const SkipButton = ({ isLight, style, ...props }) =>
  <TextButton {...props} textStyle={{ ...style, ...getDefaultStyle(isLight) }}>
    Skip
  </TextButton>;
...
const Paginator = ({ isLight, overlay, showSkip, showNext, showDone, pages, currentPage, onEnd, onNext, buttonStyle }) =>
  <View style={{ ...styles.container, ...(overlay ? styles.containerOverlay : {}) }}>
    <View style={styles.buttonLeft}>
      {showSkip && currentPage + 1 !== pages ? <SkipButton style={buttonStyle} isLight={isLight} size={BUTTON_SIZE} onPress={onEnd} /> : null}
    </View>
    <PageDots isLight={isLight} pages={pages} currentPage={currentPage} />
    <View style={styles.buttonRight}>
      {currentPage + 1 === pages ? (showDone ? <DoneButton isLight={isLight} size={BUTTON_SIZE} onPress={onEnd} /> : null) : showNext ? <NextButton isLight={isLight} size={BUTTON_SIZE} onPress={onNext} /> : null}
    </View>
  </View>;
